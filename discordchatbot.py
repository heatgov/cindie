...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="use /cindie >_<"))
    print("The bot is ready!")
@bot.command(pass_context = True)
async def cindie(ctx,*,message):
    result = chat.respond(message)
    if(len(result)<=2048):
        embed=discord.Embed(title="Cindie", description = result, color = (0xF48D1))
        ...
        for num, item in enumerate(embedList, start = 1):
            if(num == 1):
                embed = discord.Embed(title="Cindie", description = item, color = (0xF48D1))
